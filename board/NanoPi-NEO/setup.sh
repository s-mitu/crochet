...
npi_use_dts="as-is"
if [ x"$npi_use_dts" = xworkaround ] ; then
    npi_dts_file_base=npione-workaround
    npi_dts_dir=${BOARDDIR}
else
...
make_workaround_fdt ( ) {
    ...
    cmd=`echo MACHINE=arm /usr/src/sys/tools/fdt/make_dtb.sh \
    	      ${FREEBSD_SRC}/sys ${npi_dts_full_path} \
	      ${WORKDIR}/npi`
    echo === Running: $cmd ===
    sh -c "$cmd"
    if [ $? != 0 ] ; then
...
copy_workaround_fdt ( ) {
    ...
    echo === Copy dtb file to ${destdir}/sun8i-h3-nanopi-neo.dtb ===
    cp ${WORKDIR}/npi/${npi_dts_file_base}.dtb \
       ${destdir}/sun8i-h3-nanopi-neo.dtb
}

#strategy_add $PHASE_BOOT_INSTALL freebsd_install_fdt \
#	     ../gnu/dts/arm/sun8i-h3-nanopi-neo.dts \
#	     ${BOARD_BOOT_MOUNTPOINT}/sun8i-h3-nanopi-neo.dtb
#strategy_add $PHASE_FREEBSD_BOARD_INSTALL freebsd_install_fdt \
#	     ../gnu/dts/arm/sun8i-h3-nanopi-neo.dts \
#	     ${BOARD_FREEBSD_MOUNTPOINT}/boot/dtb/nanopi-neo.dtb
